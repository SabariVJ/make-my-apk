      ...
      <div className="relative overflow-hidden rounded-2xl svj-border bg-[#17171A] p-5">
        ...
        <div className="relative">
          ...
          <p className="font-inter text-sm text-[#8C8C90] mt-1">
            Track every lift. Every set feeds your <span className="text-[#C81E3A] font-semibold">Physical</span> stat.
          </p>
        </div>
      </div>
          ...
          >
            ...
            {exercises.map((ex, exIdx) => (
              <div key={ex.id} className="rounded-2xl bg-[#17171A] svj-border p-4 space-y-3">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs text-[#C81E3A]">{String(exIdx + 1).padStart(2, '0')}</span>
                  <input
                    value={ex.name}
                ...
                <button
                  onClick={() =>
                    updateExercise(ex.id, {
                      sets: [...ex.sets, { ...(ex.sets[ex.sets.length - 1] ?? { reps: 8, weight: 20 }) }],
                    })
                  }
            ...
            <div className="rounded-2xl bg-[#17171A] svj-border p-4 flex items-center justify-between">
              <div className="font-inter text-sm text-[#8C8C90]">
                ...
                <span className="text-[#F4F2ED] font-semibold">{Math.round(totals.volume).toLocaleString()}</span> kg volume
              </div>
                ...
                <div className="flex items-end gap-2 h-28">
                  {trendData.map((d, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1 min-w-0">
                      ...
                      />
                      <span className="font-mono text-[9px] text-[#8C8C90] truncate w-full text-center">{d.date}</span>
                    </div>
                  ))}
                </div>
              </div>
