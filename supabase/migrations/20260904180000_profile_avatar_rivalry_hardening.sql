-- ============================================================================
-- Profile, avatar, public-directory and rivalry hardening.
--
-- This migration is additive and idempotent. It deliberately preserves
-- existing profiles, XP, memberships, 60-Day progress and rivalry history.
-- ============================================================================
...
-- Keep progression and entitlement fields server-authoritative. Avatar changes
-- are allowed only to the current user's own avatar namespace; the Storage RLS
-- policies below enforce ownership of the actual object.
...
-- Canonical self-profile mutation. This is intentionally the only app-level
-- profile writer: it validates names and accepts avatar URLs only from the
-- authenticated user's own Storage namespace.
...
-- Public surfaces use this function instead of friendship snapshots or local
-- seed data. It deliberately exposes only public identity/progression fields.
...
-- ── Assessment baseline ─────────────────────────────────────────────────────
-- The first completed assessment creates an immutable baseline. Later activity
-- can grow the current stats, but re-saving answers cannot erase that progress.
...
-- Client sessions keep read access only. Creation, response, cancellation and
-- event scoring are trusted server/database operations with explicit caller
-- checks. Rivalry score is the event delta since the rivalry began, never
-- lifetime profile XP.
