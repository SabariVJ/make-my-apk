...
test("the homepage shows a compact Activity card before the daily task list", () => {
  ...
  const tasksIdx = challengesView.indexOf(`<SVJSectionHeader title="Today's tasks"`);
  assert.ok(cardIdx > -1 && tasksIdx > -1);
  assert.ok(cardIdx < tasksIdx, "activity card must render before today's task list");
...
test("Activity V2 maps expose real user-controlled zoom, pan and recenter", () => {
  ...
  assert.match(recordsView, /ResizeObserver/);
  ...
  assert.match(activityMap, /ResizeObserver/);
  assert.match(activityMap, /onPointerMove/);
  assert.match(activityMap, /const latest = points\[points\.length - 1\]/);
  assert.match(activityMap, /showCurrentPosition && latest/);
