        ...
        setup(builder) {
          ...
          builder.onLoad({ filter: /.*/, namespace: "mock" }, ({ path: target }) => ({
            ...
            contents: {
              ...
              motion: `import React from 'react';const cache={};export const motion=new Proxy({}, {get:(_,tag)=>cache[tag]??=(props)=>{const {children,initial,animate,transition,whileHover,whileTap,layoutId,...rest}=props;return React.createElement(tag,rest,children)}});`,
            }[target],
          }));
        },
      },
