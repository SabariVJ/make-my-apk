...
/** Mirrors UtilityNav.visibleUtilityItems(): all utility routes ship everywhere. */
...
describe("AppContent — Android utility routes", () => {
  it("does not reset Leaderboard away from Android users", () => {
    const isAndroid = true;
    let tab = "leaderboard";
    if (isAndroid && false) tab = "challenges";
    assert.equal(tab, "leaderboard");
  });
});
