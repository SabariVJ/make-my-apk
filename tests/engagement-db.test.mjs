...
if (process.env.SVJ_REWARD_REQUIRE_NATIVE === "true" && !native) {
  throw new Error(
    "Native PostgreSQL is required for this command; use the ephemeral CI database URL.",
  );
}
if (native) {
...
describe(
  "Earned Plus SQL on " + (native ? "native PostgreSQL" : "isolated PostgreSQL/WASM"),
  { concurrency: false },
  () => {
    it("ships disabled, applies twice without enabling or altering existing XP, and GET is read-only", async () => {
      ...
      assert.equal(
        (await execute("SELECT count(*)::int AS n FROM public.reward_wallets")).rows[0].n,
        0,
      );
    ...
    it("uses the explicit policy timezone regardless of database session timezone", async () => {
      ...
      const expected = await execute(
        "SELECT (clock_timestamp() AT TIME ZONE reward_timezone)::date::text AS day FROM public.reward_policies WHERE campaign_id=$1",
        [campaign],
      );
    ...
    it("grants one login receipt across retries/new request IDs without redeemable credit", async () => {
      ...
      const rows = await execute(
        "SELECT count(*)::int AS n FROM public.reward_operations WHERE user_id=$1",
        [id],
      );
    ...
    it("rejects unverified accounts before writing any wallet or receipt", async () => {
      ...
      assert.equal(
        (
          await execute("SELECT count(*)::int AS n FROM public.reward_wallets WHERE user_id=$1", [
            id,
          ])
        ).rows[0].n,
        0,
    ...
    it("requires a server session, minimum time, and a completion reflection", async () => {
      ...
      assert.equal(
        finished.state.missions.find((x) => x.assignmentId === assignment).status,
        "completed",
      );
    ...
    it("permits only one active mission, resumes its receipt, and caps a day at 150 Reward XP", async () => {
      ...
      assert.equal(
        (await start(id, "intentional-movement")).receipt.assignmentId,
    ...
    it("keeps reward tables and RPCs private while RLS hides another user's wallet", async () => {
      ...
      const hidden = await asRole(
        "authenticated",
        second,
        "SELECT user_id FROM public.reward_wallets WHERE user_id=$1",
        [first],
      );
      ...
      await assert.rejects(
        asRole("authenticated", first, "INSERT INTO public.reward_wallets(user_id) VALUES ($1)", [
          first,
        ]),
    ...
    it("expires an unfinished session and never credits it after the daily reset", async () => {
      ...
      assert.equal(result.wallet.qualifyingDays, 0);
      assert.equal(
        result.missions.find((mission) => mission.assignmentId === begun.receipt.assignmentId)
          .status,
        "expired",
      );
    });
    ...
    it("checks every claim requirement on the server before spending Reward XP", async () => {
      ...
      for (const id of [tooNew, tooFewDays, tooFewXp]) {
        ...
        assert.equal(
          (
            await execute(
              "SELECT count(*)::int AS n FROM public.reward_redemptions WHERE user_id=$1",
              [id],
            )
          ).rows[0].n,
          0,
    ...
    it("redeems once atomically, preserves Profile XP, and replays the original receipt", async () => {
      ...
      assert.equal(
        (
          await execute(
            "SELECT reward_xp,total_xp,is_plus_member FROM public.reward_wallets w JOIN public.profiles p ON p.id=w.user_id WHERE w.user_id=$1",
            [id],
          )
        ).rows[0].reward_xp,
        0,
    ...
    it("extends an existing timed Plus period and never replaces Founder lifetime access", async () => {
      ...
      const unchanged = (
        await execute(
          "SELECT reward_xp,plus_expires_at FROM public.reward_wallets w JOIN public.profiles p ON p.id=w.user_id WHERE w.user_id=$1",
          [founder],
        )
      ).rows[0];
    ...
    it("allows only one redemption for a verified identity shared by two accounts", async () => {
      ...
      assert.equal(
        (
          await execute(
            "SELECT count(*)::int AS n FROM public.reward_redemptions WHERE verified_identity=$1",
            [email],
          )
        ).rows[0].n,
        1,
    ...
    it("refuses a corrupted wallet or ledger and leaves all claim state untouched", async () => {
      ...
      const row = (
        await execute("SELECT reward_xp FROM public.reward_wallets WHERE user_id=$1", [id])
      ).rows[0];
    ...
    `);
      ...
      await execScript(
        "DROP TRIGGER test_block_earned_plus_profile ON public.profiles; DROP FUNCTION public.test_block_earned_plus_profile();",
      );
    ...
    it("cascades reward receipts when a disposable account is deleted", async () => {
      ...
      for (const [table, column] of [
        ["profiles", "id"],
        ["reward_wallets", "user_id"],
        ["reward_daily_checkins", "user_id"],
        ["reward_mission_assignments", "user_id"],
        ["reward_mission_sessions", "user_id"],
        ["reward_xp_ledger", "user_id"],
        ["reward_operations", "user_id"],
      ...
      assert.equal(
        (
          await execute(
            "SELECT count(*)::int AS n FROM public.reward_mission_sessions WHERE assignment_id=$1",
            [begun.receipt.assignmentId],
          )
        ).rows[0].n,
        0,
    ...
    it(
      "serializes verified-identity redemption races on native PostgreSQL",
      { skip: !native },
      async () => {
        ...
        assert.match(
          String(rejected?.reason?.message ?? rejected?.reason),
          /SVJ_REWARD_ALREADY_REDEEMED/,
        );
        assert.equal(
          (
            await execute(
